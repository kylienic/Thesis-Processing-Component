...
image(input,0+frame_size,0+frame_size-2);
for (int i=0; i<input.width; i++) {

  for (int j=0; j<input.height; j++) {

    hist[int(red(input.get(i, j)))]++; 
...
for (int i=0; i<input.width; i++) {
  ...
  hist[i] = int(hist[i]/maxval * input.height);
...
for (int i=0; i<input.width; i+=2) {
  ...
  line(i, input.height, i, (input.height)-hist[i]);
